...
def call_hazard(directory_hazard, scenario, year, uncertainty_variable='all', kanton=None, sun_protection=False,
                working_hours=None, only_outside=False):
    ...
    TASMAX = xr.open_dataset(tasmax_nc).sel(
        time=slice(''.join([str(year + ny), '-01-01']), ''.join([str(year + 1 + ny), '-01-01'])))  # open as xr dataset
    TASMIN = xr.open_dataset(tasmin_nc).sel(
        time=slice(''.join([str(year + ny), '-01-01']), ''.join([str(year + 1 + ny), '-01-01'])))
    ...
    TASMIN = TASMIN.where(TASMAX['tasmax'] > 22).dropna(dim='time',
                                                        how='all')  # replace all values under 22 degrees for tasmax by nas in both TASMIN and TASMAX
        ...
        if week_day < 6:  # ignores 2 out of 7 days (weekend)
            ...
            if h <= working_hours[1] or h > working_hours[2]:
                ...
                if h_min <= h < h_max:
                    ...
                    dates_out.append(TASMAX.tasmax.time[d])
                ...
                if h_max <= h:
                    ...
                    dates_out.append(TASMAX.tasmax.time[d].toordinal())
                ...
                if not only_outside:
                    ...
                    if uncertainty_variable == 'temp_in' or uncertainty_variable == 'all':
                        ...
                        temp_in[t, :, :] = temp[t, :, :] * (1 + (np.array(T_diff_fit(h,
                                                                                     *Tin_est))) * error_temp_in)  # transform to inside temperature to make the wbgt>22 test for inside events
    ...
    for i in range(len(wbgt_in)):
        l_out[i] = np.any(wbgt_out[i, :, :] > 22)
        l_in[i] = np.any(wbgt_in[i, :, :] > 22)

    wbgt_out = wbgt_out[l_out == True]
    dates_out = dates_out[l_out == True]
    ...
    return (HAZARD, i, j, dates_out)  # return the hazards, the number of events inside and outside
