...
for w in exposures_name:
    ...
    if kanton:
        shp_dir = \
            '../../input_data/shapefiles/KANTONS_projected_espg4326/swissBOUNDARIES3D_1_3_TLM_KANTONSGEBIET_espg4326.shp'
        kantons = gdb.read_file(shp_dir)

        crs = {'init': 'epsg:2056'}
        geometry = [Point(xy) for xy in zip(workers_sum_intensity.longitude, workers_sum_intensity.latitude)]
        workers_sum_intensity = gdb.GeoDataFrame(workers_sum_intensity, crs=crs, geometry=geometry)
        workers_sum_intensity = workers_sum_intensity.to_crs({'init': 'epsg:4326'})
        workers_sum_intensity = gdb.sjoin(workers_sum_intensity, kantons[['NAME', 'geometry']], how='left',
                                          op='intersects')
        workers_sum_intensity = workers_sum_intensity[workers_sum_intensity['NAME'] == kanton]
        workers_sum_intensity = workers_sum_intensity.drop(columns=['NAME', 'index_right', 'longitude', 'latitude'])
        ...
        workers_sum_intensity.value_unit = 'CHF'
        workers_sum_intensity['if_HW'] = np.full((n_exp), if_ref[w], dtype=int)
        workers_sum_intensity.crs = {'init': 'EPSG:2056'}
        workers_sum_intensity.check()
        workers_sum_intensity.fillna(0)
        workers_sum_intensity.to_crs(epsg=4326, inplace=True)
    name = w
